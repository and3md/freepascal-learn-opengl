...
var
  ...
  yaw: Single = -90.0; // yaw is initialized to -90.0 degrees since a yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit to the left.
  pitch: Single = 0.0;
  lastX: Single = 800.0 / 2.0;
  lastY: Single = 600.0 / 2.0;
  fov: Single = 45.0;
...
begin
  ...
  cameraFront := NormalizeVector3(front);
end;
...
begin
  ...
  // -------------------------------------
  ourShader := TShader.Create('7.3.camera.vs', '7.3.camera.fs');
  ...
  begin
    ...
    projection := Perspective(DegToRad(fov), SCR_WIDTH / SCR_HEIGHT, 0.1, 100);
