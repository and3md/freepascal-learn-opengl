program transformations_exercise1;
...
uses
  ...
  cthreads,
  {$ENDIF}{$ENDIF}
  Classes, SysUtils, glfw31, gl, GLext, shader_s, UMyFPImage, matrix, UMyMatrixExt, Math;
...
begin
  ...
  // -------------------------------------
  ourShader := TShader.Create('5.1.transform.vs', '5.1.transform.fs');
  ...
  begin
    ...
    transform.init_identity; // make sure to initialize matrix to identity matrix first

    // uncomment to print matrix in terminal
    //PrintMatrix4('transform', transform);

    transform := RotateMatrix4(transform, Single(glfwGetTime), Vector3(0, 0, 1.0)); // switched the order

    // uncomment to print matrix in terminal
    //PrintMatrix4('transform after rotate', transform);

    transform := TranslateMatrix4(transform, Vector3(0.5, -0.5, 0)); // switched the order
